...
def fetch_series(engine, geoid, table_id: str, variable_code: str, start_year: int, end_year: int) -> dict:
    """Returns {year: estimate} for a single variable across a year range.
    geoid may be a single geoid (str) or a list of geoids -- when a list,
    values are SUMMED across geographies in SQL before any downstream
    rate/percentage math (CAGR, turnover, joint distribution), same
    principle as demographics_dashboard.py's fetch_multi(): sum raw counts
    first, never average or sum a rate/percentage across geographies."""
    is_region = isinstance(geoid, (list, tuple))
    with engine.connect() as conn:
        if is_region:
            rows = conn.execute(
                text("""
                    SELECT year, SUM(estimate) AS estimate FROM acs_estimates
                    WHERE geoid = ANY(:geoids) AND table_id = :table_id AND variable_code = :var
                      AND year BETWEEN :start AND :end
                    GROUP BY year
                """),
                {"geoids": list(geoid), "table_id": table_id, "var": variable_code, "start": start_year, "end": end_year},
            )
        else:
            rows = conn.execute(
                text("""
...
def project_population_and_households(engine, geoid, base_year: int, target_year: int,
    ...
    geoid may be a single geoid or a list -- population and households are
    raw counts, safely summed across geographies by fetch_series(). Average
    household size (AVG_HH_SIZE_VAR) is NOT a count, it's itself an average
    -- summing it across geoids the way fetch_series() sums counts would be
    an "average of averages," the same class of error already fixed for
    DP05/S1901/S2501 region aggregation elsewhere in this project. There's
    also no Census-published "region average household size" to fall back
    on, so for a region it's derived as region population / region
    households instead -- population divided by households is literally
    how average household size is defined, so this is a valid population-
    weighted regional average, not an approximation."""
    is_region = isinstance(geoid, (list, tuple))
    ...
    if is_region:
        hh_size_series = {
            y: pop_series[y] / actual_households[y]
            for y in pop_series if y in actual_households and actual_households[y]
        }
    else:
...
def _fetch_b19037_raw(engine, geoid, start_year: int, end_year: int) -> dict:
    ...
    geoid may be a single geoid or a list -- when a list, sums raw B19037
    estimates across geographies in SQL, grouped by (year, variable_label)
    since a given year's vintage uses the same label format across every
    geoid (B19037 has no per-geoid duplicate-label ambiguity the way DP05
    does, verified empirically), so the joint-distribution math in
    compute_b19037_breakdown() operates on valid region totals."""
    is_region = isinstance(geoid, (list, tuple))
    with engine.connect() as conn:
        if is_region:
            rows = conn.execute(
                text("""
                    SELECT year, variable_label, SUM(estimate) AS estimate FROM acs_estimates
                    WHERE geoid = ANY(:geoids) AND table_id = 'B19037' AND year BETWEEN :start AND :end
                    GROUP BY year, variable_label
                """),
...
def compute_b19037_breakdown(engine, geoid, base_year: int, target_year: int,
...
def project_housing_demand(geoid, base_year: int, target_year: int, *,
                            ...
                            engine=None) -> dict:
    """Top-level entry point. geoid may be a single geoid or a list of
    geoids -- every DB fetch beneath this sums across geographies in SQL
    before any rate/percentage math runs, so a region's population/
    household-size CAGR, turnover demand, and age x income breakdown are
    all computed on valid region totals, the same "sum raw counts first"
    principle used throughout demographics_dashboard.py. rate_basis params
    are "5yr", "10yr", or "custom". turnover_tier is a key in
    TURNOVER_TIERS, or "custom". b19037_demand_basis is "annual" or
    "total" -- which demand figure the age x income matrix sums to."""
