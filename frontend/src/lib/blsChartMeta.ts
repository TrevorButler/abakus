...
import { BLS_SECTORS } from './api'
import type { ValueFormat } from './chartMeta'

const SECTOR_LABELS: Record<string, string> = Object.fromEntries(BLS_SECTORS.map((s) => [s.code, s.label]))
...
export function blsChartMeta(key: string): BlsChartMeta {
  if (key === 'employment_by_sector') return { title: 'Employment by Sector', format: 'percent' }
  if (key === 'wages_by_sector') return { title: 'Wages by Sector', format: 'percent' }
