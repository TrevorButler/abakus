...
import { seriesRows, ageIncomeRows, type SheetData } from '../lib/download'
...
export default function HousingDemand() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 py-12 gap-8">
      ...
      {/* Desktop-first: once a geography (or region) is selected, the
          picker moves to the left and the assumptions form sits alongside
          it on the right rather than stacking everything in one narrow
          column. Results render full-width below, separately. */}
      <div className={`w-full ${selectedGeoids.length > 0 ? 'max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-start' : 'max-w-lg flex justify-center'}`}>
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="self-center">
        <DownloadSheetsButton filename="Housing Demand Projection.xlsx" sheets={sheets} />
      </div>
    </div>
  )
}
